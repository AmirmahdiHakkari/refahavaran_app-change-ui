  ...
  return (
    <NoSsr fallback={fallback ? renderFallback : null}>
      <Box
        ...
        {...other}
      >
        {scrollSnaps.slice().reverse().map((_, displayIndex) => {
          const sourceIndex = scrollSnaps.length - 1 - displayIndex;
          const selected = sourceIndex === selectedIndex;

          return (
            <Box component="li" key={sourceIndex} sx={{ display: 'inline-flex' }}>
              <ButtonBase
                disableRipple
                aria-label={`dot-${sourceIndex}`}
                className={carouselClasses.dot.concat(
                  selected ? ` ${carouselClasses.state.selected}` : ''
                )}
                onClick={() => onClickDot(sourceIndex)}
                sx={{
